...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.loginService.checkSession(key).subscribe(
      (res) => {
        this.dataaux = res;
        this.postService.getPost(this.dataaux.IdUsuario).subscribe(
          (res) => {
            this.data2 = res;
          },
          (err) => console.log(err)
  ...
  eliminar() {
    this.postService.deletePost(this.idPub).subscribe(
      (res) => {
  ...
  editar() {
    this.postService.putId(this.idPub);
  ...
  mostrarComentario(idPost: number) {
    if (this.auxMostrarOcultar || this.auxComentarioAnterior != idPost) {
      ...
      this.activar = true;
      this.ComentarioService.listarComentarios(idPost,this.limitComentario).subscribe((res) => {
